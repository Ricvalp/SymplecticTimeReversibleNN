...
#CHECKPOINT 1,2,3
# checkpoint_path_1R = "checkpoints_SN_PP_1R_008/cp.ckpt"
# checkpoint_dir_1R = os.path.dirname(checkpoint_path_1R)

# checkpoint_path_2R = "checkpoints_SN_PP_2R_008/cp.ckpt"
# checkpoint_dir_2R = os.path.dirname(checkpoint_path_2R)

# checkpoint_path_1N = "checkpoints_SN_PP_1N_008/cp.ckpt"
# checkpoint_dir_1N = os.path.dirname(checkpoint_path_1N)

# checkpoint_path_2N = "checkpoints_SN_PP_2N_008/cp.ckpt"
...
f = open("HM_SN_Loss.txt", "a")
for i in history1N2.history["loss"]:
        f.write(str(i))
        f.write("\n")
f.close()

f = open("HM_SN_Val.txt", "a")
...
plt.plot(history1N2.history["loss"], label="Training loss $\mathbf{H}'_{S}$")
plt.plot(history1N2.history["val_loss"], label="Validation loss $\mathbf{H}'_{R}$")
...
f_iterations = [model1N2(x0)]
for _ in range(100):
    f_iterations.append(model1N2(f_iterations[-1]))

b_iterations = [f_iterations[-1]*np.array([[1., -1.]])]

for _ in range(100):
    b_iterations.append(model1N2(b_iterations[-1]))
...
iterations1N2 = [model1N2(x0)]
for _ in range(N_predictions):
    iterations1N2.append(model1N2(iterations1N2[-1]))
...
f = open("SN_PP_Prediction1N2.txt", "a")
for i in iterations1N2:
    for j in i.numpy():
        f.write(str(j[0]))
        f.write("\n")
        f.write(str(j[1]))
