...
model1R = rs.Henon(25, 'reversible')
...
#CHECKPOINT 1,2,3
checkpoint_path_1R = "checkpoints_HM_HH_1R_008/cp.ckpt"
checkpoint_dir_1R = os.path.dirname(checkpoint_path_1R)

checkpoint_path_2R = "checkpoints_HM_HH_2R_008/cp.ckpt"
checkpoint_dir_2R = os.path.dirname(checkpoint_path_2R)

#checkpoint_path_1N = "checkpoints_HM_HH_1N_008/cp.ckpt"
#checkpoint_dir_1N = os.path.dirname(checkpoint_path_1N)

#checkpoint_path_2N = "checkpoints_HM_HH_2N_008/cp.ckpt"
#checkpoint_dir_2N = os.path.dirname(checkpoint_path_2N)

#checkpoint_path_1N2 = "checkpoints_HM_HH_1N2_008/cp.ckpt"
...
iterations = [model1R(x0)]
for _ in range(200):
    iterations.append(model1R(iterations[-1]))
...
f_iterations = [model1R(x0)]
for _ in range(100):
    f_iterations.append(model1R(f_iterations[-1]))

b_iterations = [f_iterations[-1]*np.array([[1., -1.]])]

for _ in range(100):
    b_iterations.append(model1R(b_iterations[-1]))
...
iterations1R = [model1R(x0)]
for _ in range(N_predictions):
    iterations1R.append(model1R(iterations1R[-1]))
...
f = open("HM_HH_Prediction1R.txt", "a")
for i in iterations1R:
    for j in i.numpy():
        f.write(str(j[0]))
        f.write("\n")
        f.write(str(j[1]))
...
f = open("HM_HH_Prediction2R.txt", "a")
